...
namespace Behzadbabaei\Translation\Repositories;

use Illuminate\Database\Query\JoinClause;
use Illuminate\Foundation\Application;
use Illuminate\Support\Arr;
use Illuminate\Support\NamespacedItemResolver;
use Behzadbabaei\Translation\Models\Translation;
...
class TranslationRepository extends Repository
{
     ...
     *
     * @var \Behzadbabaei\Translation\Models\Translation
     */
    protected $model;
     ...
     *  Constructor
     *
     * @param \Behzadbabaei\Translation\Models\Translation $model     Bade model for queries.
    ...
    /**
     *  Update and lock translation. Locked translations will not be ovewritten when loading translation files into the
     *  database. This will force and update if the translation is locked. If the attributes are not valid, a null
     *  response is given and the errors can be retrieved through validationErrors()
     *
     * @param array $attributes Model attributes
     *
     * @return boolean
    ...
    public function updateDefaultByCode($code, $text)
    {
        ...
        $translation = $this->model->whereLocale($locale)->whereNamespace($namespace)->whereGroup($group)
            ->whereItem($item)->first();
    ...
    /**
     *  Delete a translation. If the translation is of the default language, delete all translations with the same
    ...
    public function delete($id)
    {
        ...
        if ($translation->locale === $this->defaultLocale) {
            return $this->model->whereNamespace($translation->namespace)->whereGroup($translation->group)
                ->whereItem($translation->item)->delete();
        } else {
    ...
    /**
     *  Return a list of translations for the given language. If perPage is > 0 a paginated list is returned with
        ...
        foreach ($elements as $element) {
            if ($element) {
                $query = $query->where(function ($query) use ($element) {
                    $query->where('group', 'like', "%{$element}%")->orWhere('item', 'like', "%{$element}%")
                        ->orWhere('text', 'like', "%{$element}%");
                });
            }
        }
    ...
    public function untranslated($locale, $perPage = 0, $text = null)
    {
        ...
        $untranslated = $text ? $this->model->whereIn('id', $ids)
            ->where('text', 'like', "%$text%") : $this->model->whereIn('id', $ids);
    ...
    public function findByLangCode($locale, $code)
    {
        ...
        return $this->model->whereLocale($locale)->whereNamespace($namespace)->whereGroup($group)->whereItem($item)
            ->first();
    }
    ...
    {
        $this->model->whereNamespace($namespace)->whereGroup($group)->whereItem($item)
            ->where('locale', '!=', $this->defaultLocale)->update(['unstable' => '1']);
    }
